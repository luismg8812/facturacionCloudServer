...
class DocumentoControllers{
    ...
    public async createDocumento (req:Request, res:Response):Promise<any>{            
        ...
        await db.query(query, [tipo_documento_id, empresa_id, proveedor_id, usuario_id, cliente_id, empleado_id, fecha_registro, consecutivo_dian,impreso,total,excento,gravado,iva,cierre_diario,detalle_entrada,mac,saldo,peso_total,descuento, cambio,iva_5,iva_19,base_5,base_19,retefuente,interes,total_costo,letra_consecutivo,invoice,anulado,documento_id,fecha_entrega,descripcion_cliente,descripcion_trabajador,modelo_marca_id,linea_vehiculo,impresora]).then(res2=>{
    ...
    public async getNominaByEmpleado(req:Request, res:Response):Promise<any>{
        const fechaInicial = req.query.fechaInicial; 
        const fechaFinal = req.query.fechaFinal; 
        let query:string ="select subt.nombre, vale.empleado_id, coalesce(subt.subtotal, 0) subtotal , coalesce(vale.vales, 0) vales ,coalesce(produ.productos, 0) productos ,coalesce(subt.pago_admin, 0)  admon, (coalesce(subt.subtotal, 0)- coalesce(vale.vales, 0) -coalesce(produ.productos, 0)+coalesce(subt.pago_admin, 0) ) total from"
